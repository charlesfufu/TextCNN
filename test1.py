...
class TextCNN(object):  # 定义了1个TEXTCNN的类，包含一张大的graph
    ...
    def __init__(
            self, sequence_length=12859, num_classes=6,
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):  # 封装了一个叫做“embedding'的模块，使用设备cpu，模块里3个operation
            self.W = tf.Variable(ALLword2vec,name="W")  # operation1，一个（词典长度*embedsize）tensor，作为W，也就是最后的词向量
...
if __name__ == "__main__":
    ...
    for i in range(len(Vocabulary)):
        ALLword2vec.append(model_2[Vocabulary[i]])
    ALLword2vec = np.array(ALLword2vec)
    print(ALLword2vec)
